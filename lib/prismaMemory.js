...
export function createMemoryClient() {
  return {
    ...
    application: {
      async count({ where } = {}) {
        return db.applications.filter((r) => matches(r, where)).length;
      },
      async findFirst({ where, select } = {}) {
        const row = db.applications.find((a) => matches(a, where));
        return row ? pick(row, select) : null;
      },
      async findMany({ where, orderBy, take, include } = {}) {
        let rows = sortBy(db.applications.filter((a) => matches(a, where)), orderBy);
        if (take != null) rows = rows.slice(0, take);
        return rows.map((r) => withUser(r, include));
      },
      async create({ data, include } = {}) {
      ...
      async delete({ where } = {}) {
        const idx = db.applications.findIndex((a) => matches(a, where));
        if (idx === -1) {
          const err = new Error("Solicitação não encontrada.");
          err.code = "P2025";
          throw err;
